...
import { INode, IPicto, ISelection, IVariable } from "src/types";
...
export default class Pictos {
  ...
  private getVariableNodes = (variableName:Pictos, variables:IVariable[]): Pictos => {
    const variable = variables.find((v) => v.varName.equals(variableName));
    if (!variable) {
      return;
    }
